...
img_filename = "/home/pi/picam_vids/vid_pic_"+time_file+".jpg"
vid_filename = "/home/pi/picam_vids/vid_"+time_file+".h264"
mp4_filename = "/home/pi/picam_vids/vid_"+time_file+".mp4"

starttime = strftime("%Y-%m-%d %H:%M:%S")
iname=0

print("Starting preview...please wait...")
with picamera.PiCamera() as camera: # shiould include as a function if a preview is needed with each loop
    camera.rotation = 180
...
while True:
    ...
    #if (int(var) == 1):
    if(GPIO.input(24) ==1): # this would be the trigger input pin
...
##            camera.start_preview()
            ...
            GPIO.wait_for_edge(24, GPIO.FALLING) # time.sleep(xsec) # time for testing or recording duration (s)
...
##            camera.start_preview()
            ...
            print("     Wating for start trigger... (press ctrl-C to end)")
